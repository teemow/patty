...
func TestRepoFindsTokensInsideSecretManifests(t *testing.T) {
	...
	res, err := Repo(ctx, "fixture", repo, nil, Options{Workers: 1})
	if err != nil {
		t.Fatal(err)
	}
	...
	res, err = Repo(ctx, "fixture", repo, nil, Options{Workers: 1, Ignore: map[string]bool{string(kubernetes.KindSecretManifest): true}})
	if err != nil {
		t.Fatal(err)
	}
