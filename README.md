...
<p align="center">
  Finds leaked GitHub and Slack tokens in every corner of a repository's history.<br>
...
- `--verify` -- ask GitHub and Slack which tokens are still **active**; `--revoke` then revokes those, after asking (`--yes` skips the question)
...
3. **Scan objects, not diffs.** Every blob, commit and tag in the object database is read exactly once, reachable or not, and checked for GitHub tokens and Slack tokens and webhooks. Classic GitHub tokens are confirmed against their built-in checksum, so a `ghp_` lookalike in a test fixture is not reported.
...
Test tokens are constructed at runtime -- classic GitHub tokens from a random part plus a computed checksum, Slack tokens from their id groups and secret -- so no token-shaped string is committed to this repository.
