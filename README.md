...
`--verify` sends each found token to `GET /user` (or `GET /installation/repositories` for app installation tokens) to see whether GitHub still accepts it. `--revoke` and `patty revoke` send tokens to `POST /credentials/revoke`, GitHub's unauthenticated endpoint for reporting leaked credentials. Nothing else ever leaves your machine; the tokens patty finds are not sent anywhere unless you ask for that.
...
| `--revoke` | | `false` | Ask GitHub to revoke every active token found (implies `--verify`; see [Revoking](#revoking)) |
| `--yes` | `-y` | `false` | Revoke without asking for confirmation |
| `--json` | | `false` | Print the report as JSON |
| `--show-secrets` | | `false` | Print full token values (the default shows `ghp_AbCdEfGh…WxYz`) |
| `--all-refs` | | `false` | List every branch, tag and pull request a commit is on instead of the first five |
...
```bash
patty revoke [token...]   # Ask GitHub to revoke tokens you have in hand (stdin when no arguments)
...
- A token that is still configured on this machine (the *local* line) stops working for that tool the moment it is revoked. Have the replacement ready. Afterwards each token is checked again: GitHub processes revocations asynchronously, so a token may still answer as *pending* for a moment. In the report and the JSON (`"revocation": "revoked" | "pending"`) that is recorded per token.
